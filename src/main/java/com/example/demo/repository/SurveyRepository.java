...
import org.springframework.stereotype.Repository; // ★ SpringのBeanとして登録するためのインポート

import java.sql.*;

@Repository // ★ これをつけることで Spring Boot がこのクラスを認識できるようになります
public class SurveyRepository {
    ...
    public boolean insert(SurveyData d) {
        String sql = "INSERT INTO survey_data (user_id, gender, age, height_cm, weight_kg, bmi, goal_weight_kg, " +
                     "goal_date, remain_days, purpose, purpose_other, activity_level, job_type, diet_exp, meal_style, allergy, created_at) " +
                     "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
    ...
    /**
     * ユーザーIDでアンケートデータを取得します
     */
    public SurveyData findByUserId(int userId) {
        String sql = "SELECT * FROM survey_data WHERE user_id = ?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, userId);
    ...
    /**
     * 既存のアンケートデータを更新します（体重・身長・BMI等の更新用）
     */
    public boolean update(SurveyData d, int userId) {
        String sql = "UPDATE survey_data SET height_cm = ?, weight_kg = ?, bmi = ?, goal_weight_kg = ?, " +
                     "goal_date = ?, remain_days = ?, activity_level = ?, meal_style = ? " +
                     "WHERE user_id = ?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setDouble(1, d.getHeight());
            ps.setDouble(2, d.getWeight());
            ps.setDouble(3, d.getBmi());
            ps.setDouble(4, d.getGoalWeight());
            ps.setString(5, d.getGoalDate());
            ps.setInt(6, d.getRemainDays());
            ps.setString(7, d.getActivityLevel());
            ps.setString(8, d.getMealStyle());
            ps.setInt(9, userId);
            return ps.executeUpdate() > 0;
        } catch (SQLException e) {
            e.printStackTrace();
