 ...
 *
 * DB設定は resources/db.properties を参照。
 */
public class SurveyRepository {

    // ---------- 接続情報（実環境では JNDI DataSource 推奨） ----------
    private static final String DB_URL  = "jdbc:mysql://localhost:3306/5diet?useSSL=false&serverTimezone=Asia/Tokyo";
    private static final String DB_USER = "diet_user";
    private static final String DB_PASS = "diet_pass";

    static {
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
        } catch (ClassNotFoundException e) {
            throw new RuntimeException("MySQL JDBC Driver が見つかりません", e);
        }
    }
