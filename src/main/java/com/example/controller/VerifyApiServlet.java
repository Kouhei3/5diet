...
public class VerifyApiServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            // UserDaoの verifyAccount メソッドを呼び出す
            if (userDao.verifyAccount(username, code)) {
                response.setStatus(HttpServletResponse.SC_OK);
                jsonResponse.addProperty("status", "success");
                jsonResponse.addProperty("message", "本登録が完了しました！ログイン画面へ移動します。");
            } else {
                response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                jsonResponse.addProperty("status", "error");
                jsonResponse.addProperty("message", "認証コードが正しくないか、有効期限が切れています。");
            }
        } catch (Exception e) {
